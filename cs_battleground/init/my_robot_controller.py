...
# Разберитесь, как он работает, а потом сотрите все комментарии и doc-strings
# ИЛИ ЗАПУСТИТЕ ГЕНЕРАЦИЮ ЕЩЕ РАЗ с флагом --no-comments
# python -m cs_battleground.init --no-comments
# в этом случае сгенерируется ФАЙЛ БЕЗ КОММЕНТАРИЕВ
...
def main():
    with coppelia_sim_connection('localhost'):
        # При входе в loaded_robot, в сцену CoppeliaSim будет загружена модель my_robot
